...
def load_data_pair(input_file, word_idx, max_doc_len = 75, max_sen_len = 45):
    ...
    while True:
        ...
        y_couple.append(pairs)
        mask = [True for _ in range(d_len)] + [False for _ in range(max_doc_len-d_len)]
        y_mask.append(mask)
        doc_ids.append(doc_id_str)

    y_position, y_cause, y_pair, x, sen_len, doc_len, distance, y_mask = map(torch.tensor, \
    [y_position, y_cause, y_pair, x, sen_len, doc_len, distance, y_mask])
    y_couple, doc_ids = map(np.array, [y_couple, doc_ids])

    for var in ['y_position', 'y_cause', 'y_pair', 'x', 'sen_len', 'doc_len', 'distance', 'y_mask', 'y_couple', 'doc_ids']:
        print('{}.shape {}'.format( var, eval(var).shape ))
    print('n_cut {}'.format(n_cut))
    print('load data done!\n')
    return y_position, y_cause, y_pair, x, sen_len, doc_len, distance, y_mask, y_couple, doc_ids
