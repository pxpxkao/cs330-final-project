...
batch_size = 128                                                     # number of example per batch
...
class E2E_PextE(nn.Module):
    def __init__(self, embedding_dim, embedding_dim_pos, sen_len, doc_len, keep_prob1, keep_prob2, \
        ...
        self.pair_linear1_new = nn.Linear(400, n_hidden//2)
    ...
    def get_pair_prediction(self, x1, x2, distance):
        ...
        # print("yoyoyoyoyoyoyoyoyoyheeeee")
    ...
    def pair_prediction(self, x1, x2, distance, start, window_size):
        x = create_pairs_sliding_window(x1, x2, start, window_size)
        # print(x1.shape, x2.shape, x.shape, start, window_size)
        # x is of shape (batch_size, max_doc_len * max_doc_len, 4 * n_hidden)
        # print(x.shape, distance.shape, 4 * self.n_hidden + self.embedding_dim_pos)
        # x_distance = torch.cat([x, distance.float()], -1)
        # x_distance is of shape (batch_size, max_doc_len * max_doc_len, 4 * n_hidden + embedding_dim_pos)
        # x_distance = x_distance.reshape(-1, 4 * self.n_hidden + self.embedding_dim_pos)
        x_distance = x.reshape(-1, 400)
        x_distance = self.dropout3(x_distance)
        # print(x_distance.shape)
        # x_distance = self.pair_linear1(x_distance)
        # x is of shape (batch_size * max_doc_len * max_doc_len, 4 * n_hidden + embedding_dim_pos)
        pred_pair = F.softmax(self.pair_linear2(self.relu(self.pair_linear1_new(x_distance))), dim = -1)
        # pred_pair is of shape (batch_size * max_doc_len * max_doc_len, n_class)
        # print(pred_pair.shape)
        pred_pair = pred_pair.reshape(-1, 410, self.n_class)
        # pred_pair is of shape (batch_size, max_doc_len * max_doc_len, n_class)

        # print(pred_pair.shape)
        return pred_pair

    def forward(self, x, distance, start, window_size):
        '''
        ...
        # print(x_pos.shape, x_cause.shape)
        # pred_pair = self.get_pair_prediction(x_pos, x_cause, distance)
        
        # print("Size of pred pair:", pred_pair.shape)
        # print("Size of pred pos: ", pred_pos.shape)
        # print("Size of pred cause: ", pred_cause.shape)
        # print(x_pos.shape, x_cause.shape)

        # add
        pred_pair_row = self.pair_prediction(x_pos, x_cause, distance, start, window_size)
        # print(pred_pair.shape, pred_pair_row.shape)
        # reg += reg_tmp

        # pred_pair_col = self.pair_prediction(x_cause, x_pos, distance, start, window_size)
        # reg += reg_tmp


        return pred_pos, pred_cause, pred_pair_row
    ...
    for fold in range(1, 11):
        ...
        for epoch in range(1, training_epochs + 1):
            ...
            for train, _ in get_batch_data_pair(
                ...
                for starts in range(5, 37, 5):
                    Model.train()
                    tr_pred_y_pos, tr_pred_y_cause, tr_pred_y_pair = Model(embedding_lookup(word_embedding, \
                    tr_x_batch), embedding_lookup(pos_embedding, tr_distance_batch), starts, sliding_window_size)

                    # print(tr_true_y_pair[0])
                    # print(tr_true_y_pair[0])
                    tr_true_y_pair_window = create_true_pairs_sliding_window(tr_true_y_pair, starts, sliding_window_size, max_doc_len)
                    # print(tr_true_y_pair[0])
                    # print(starts, tr_true_y_pos.shape, tr_true_y_cause.shape, tr_true_y_pair.shape)
                    # print(tr_pred_y_pos.shape, tr_pred_y_cause.shape, tr_pred_y_pair.shape)
                    # print(tr_doc_len_batch, pos, cause, pair)
                    ...
                    pair_criterion(tr_true_y_pair_window, tr_pred_y_pair, tr_doc_len_batch)*pair
                    optimizer.zero_grad()
                    loss.backward()
                    optimizer.step()
                    ...
                    if step % 25 == 0 and starts == 5:
                        ...
                        print('cause_predict: train acc {:.4f} p {:.4f} r {:.4f} f1 score {:.4f}'.format(
                                acc, p, r, f1))
                        acc, p, r, f1 = acc_prf_pair(tr_pred_y_pair, tr_true_y_pair_window, tr_doc_len_batch)
            ...
            with torch.no_grad():
                ...
                val_x), embedding_lookup(pos_embedding, val_distance), 5, sliding_window_size)

                val_y_pair_window = create_true_pairs_sliding_window(val_y_pair, 5, sliding_window_size, max_doc_len)
                ...
                pair_criterion(val_y_pair_window, val_pred_y_pair, val_doc_len)*pair
                ...
                acc, p, r, f1 = acc_prf_pair(val_pred_y_pair, val_y_pair_window, val_doc_len)
...
def main():
    ...
    # pred_pos, pred_cause, pred_pair = Model(x, distance, 5, 5)
