...
def getmask(y, doc_len):
    ...
    max_doc_len = 41
    y_mask = torch.zeros(doc_len * 10, 2)
    # print(y.shape, max_doc_len, doc_len, y.shape, y_mask.shape)
    while j < doc_len * 10:
...
class ce_loss_pair(torch.nn.Module):
    ...
    def forward(self, y_true, y_pred, doc_len):
        ...
        loss = torch.autograd.Variable(torch.zeros([], dtype=torch.double))
        for i in range(len(doc_len)):
            # print("hey, ", y_true[i].clone().shape, doc_len[i])
...
def create_pairs_sliding_window(x1, x2, start, window_size):
    '''
    input(s) shape: [batch_size, doc_len, 2 * n_hidden]
    output shape: [batch_size, doc_len * doc_len, 4 * n_hidden]
    '''  
    ...
    for i in range(x1.shape[1]):
        # for j in range(x2.shape[1]):
        for j in range(start - window_size, start + window_size):
            x3_tmp = torch.cat([x1[:, i, :], x2[:, j, :]], dim=1).unsqueeze(1)
            if iters :
                x3 = torch.cat([x3, x3_tmp], dim=1)
            else :
...
def create_true_pairs_sliding_window(x, start, window_size, max_doc_len):
    iters = 0
    for i in range(max_doc_len):
        for j in range(start - window_size, start + window_size):
            # x3_tmp = torch.cat([x1[:, i, :], x2[:, j, :]], dim=1).unsqueeze(1)]
            # print(i, j, i * max_doc_len + j)
            x3_tmp = x[:, i * max_doc_len + j, :].unsqueeze(1)
            # print(i, j, i * max_doc_len + j)
            if iters :
                x3 = torch.cat([x3, x3_tmp], dim=1)
            else :
...
def acc_prf_aux(pred_y, true_y, doc_len, average='weighted'):
    ...
    for i in range(len(doc_len)):
        ...
        pred_indices_masked.extend(pred_indices[i, :doc_len[i]].cpu())
